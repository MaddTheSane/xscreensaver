...
# ifndef USE_IPHONE
  ...
  for (NSString *font in fonts) {
    CFURLRef url = (__bridge CFURLRef) [NSURL fileURLWithPath: font];
    CFErrorRef err = 0;
...
# endif
  ...
  CFBridgingRetain(xwindow->window.view);   // needed for garbage collection?
...
# endif
    CFRelease ((__bridge CFTypeRef)(xwindow->window.view));
    free (xwindow);
    xwindow = NULL;
...
{
  if (! dpy) return 0;
  XScreenSaverView *view = (XScreenSaverView *)(jwxyz_window_view (XRootWindow (dpy, 0)));
  if (!view) return 0;
